...
function GNLib.OpenAddonsList()
    ...
    function addon_infos:Paint(w, h)
        ...
        if selected_addon then
            ...
            --  > Draw name
            GNLib.SimpleTextShadowed(selected_addon.name .. " - " .. (selected_addon.version or "N/A"), "GNLFontB40",
                25 * 1.5 + iconSize, 25, color_white, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 2, 2, _)

            --  > Draw certification
            surface.SetFont("GNLFontB40")
            local name_w, name_h = surface.GetTextSize(selected_addon.name ..
            " - " .. (selected_addon.version or "N/A"))
            surface.SetDrawColor(color_white)
            ...
            --  > Draw Install status
            GNLib.DrawIconTextOutlined(selected_addon.installed and "Installed" or "Not Installed", "GNLFontB17",
                25 * 1.5 + iconSize, 25 + name_h,
                selected_addon.installed and GNLib.Colors.Emerald or GNLib.Colors.Alizarin,
                selected_addon.installed and mats.Accept or mats.NotAccept, 16, 16, _, _, 1)

            --  > Draw lib update status
            GNLib.DrawIconTextOutlined(
            GNLib.IsOutdatedLib(selected_addon.id) and
            "Different version of the library (" .. string.Trim(selected_addon.lib_version) .. ")" or
            "Similar version of the library", "GNLFontB17", 25 * 1.5 + iconSize, 25 + name_h + 20,
                GNLib.IsOutdatedLib(selected_addon.id) and GNLib.Colors.Orange or GNLib.Colors.Emerald,
                GNLib.IsOutdatedLib(selected_addon.id) and mats.Update or mats.Accept, 16, 16, _, _, 1)

            --  > Draw addon update status
            GNLib.DrawIconTextOutlined(
            GNLib.IsOutdated(selected_addon.id) and
            "Different version of this addon (" .. string.Trim(selected_addon.last_version) .. ")" or
            "Similar version of this addon", "GNLFontB17", 25 * 1.5 + iconSize, 25 + name_h + 40,
                GNLib.IsOutdated(selected_addon.id) and GNLib.Colors.Orange or GNLib.Colors.Emerald,
                GNLib.IsOutdated(selected_addon.id) and mats.Update or mats.Accept, 16, 16, _, _, 1)
            ...
            selected_addon.wip and "Warning ! This addon may create errors or may not work properly !" or
            "This addon is currently stable.", "GNLFontB17", 25 * 1.5 + iconSize + 20, 25 + name_h + 60,
                selected_addon.wip and GNLib.Colors.SunFlower or GNLib.Colors.Emerald, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP, 1,
                color_black)
        end
    end
    ...
    function addAddon(k, v)
        ...
        function addon_line:Paint(w, h)
            ...
            draw.SimpleText((v.author or "N/A") .. " | " .. (v.lib_version or "N/A"), "GNLFont15", 5, h - 10, color_white,
